"""
Insert friend system JS functions into index.html.
Right panel HTML already replaced. Only need to add JS and update initQuantTalk.
"""
...
# Find insertion point: before the TV widget section
insert_idx = t.find('// ---- TradingView Widget ----')
if insert_idx < 0:
    insert_idx = t.find('function loadTV(')
...
# Insert using raw bytes
encoded_js = friend_js.encode('utf-8')
r2 = r[:insert_idx] + encoded_js + r[insert_idx:]
with open(filepath, 'wb') as f:
    f.write(r2)
print(f'JS inserted: {len(encoded_js)} bytes')
print(f'File size: {len(r2)} bytes')

# Step 2: Update initQuantTalk
t2 = r2.decode('utf-8', errors='replace')
old_init = 'function initQuantTalk(){\n  loadSQData();\n  renderChannelList();\n  if(_sqChannels.length > 0) switchChannel(_sqChannels[0].id);\n  initWalletUI();\n}'
new_init = 'function initQuantTalk(){\n  loadSQData();\n  renderChannelList();\n  if(_sqChannels.length > 0) switchChannel(_sqChannels[0].id);\n  initWalletUI();\n  initFR();\n}'
if old_init in t2:
    t2 = t2.replace(old_init, new_init)
    r3 = t2.encode('utf-8', errors='replace')
    with open(filepath, 'wb') as f:
        f.write(r3)
    print('initQuantTalk updated OK')
else:
    # Search for alternative syntax
    idx = t2.find('function initQuantTalk')
    if idx > 0:
        end = t2.find('}', t2.find('initWalletUI()', idx))
        print(f'Found at {idx}-{end}: {repr(t2[idx:end+1])}')

# Syntax check
print('\n=== Syntax Check ===')
sidx = r3.find(b'<script>', 200000) if 'r3' in dir() else r2.find(b'<script>', 200000)
cs = sidx + 8
eidx = (r3 if 'r3' in dir() else r2).find(b'</script>', cs)
c = (r3 if 'r3' in dir() else r2)[cs:eidx]
import os
with open(os.path.join(os.environ.get('TEMP','/tmp'),'sc.js'), 'wb') as f:
    f.write(c)
res = subprocess.run(['node','--check',os.path.join(os.environ.get('TEMP','/tmp'),'sc.js')], capture_output=True, timeout=15)
print(f'Node check: {res.returncode}')
if res.returncode != 0:
    err_msg = res.stderr.decode('utf-8', errors='replace')[:800]
    print(f'ERROR:\n{err_msg}')
else:
    print('Syntax OK!')
