...
console.log('=== 1. 创建 local.properties ===');
...
const env = {
    ...
    PATH: `C:\\Program Files\\Eclipse Adoptium\\jdk-21.0.10.7-hotspot\\bin;${sdkPath}\\platform-tools;${sdkPath}\\cmdline-tools\\latest\\bin;${process.env.PATH}`
};

const gradlewBat = path.join(androidDir, 'gradlew.bat');
console.log(`执行: ${gradlewBat} assembleDebug`);
            ...
            if (fs.existsSync(apkDir)) {
                ...
                if (files.length > 0) {
                    files.forEach(f => {
                        ...
                        console.log(`\n🎉 APK 已生成: ${f} (${size} MB)`);
                        console.log(`   路径: ${path.join(apkDir, f)}`);
                    });
                } else {
